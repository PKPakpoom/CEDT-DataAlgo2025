...
void generate_recursive(int low, int high, CP::map_bst<int, int> &bst) {
    if (low > high) {
        return;
    }
    int median = low + (high - low) / 2; 
    bst[median] = 100;
    generate_recursive(low, median - 1, bst);
    generate_recursive(median + 1, high, bst);
}
